...
// SetMemoryProvider wires an optional tier-3 memory provider for cron-triggered runs.
func (s *Scheduler) SetMemoryProvider(provider agentctx.MemoryProvider) {
	s.memory = provider
}
...
func (s *Scheduler) runJob(job Job) {
	...
	resp, err := s.agentInstance.Run(ctx, session.ID, job.Prompt, msgProvider, s.memory)
