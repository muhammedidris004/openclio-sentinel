...
func NewServer(
	...
	mux.HandleFunc("/api/v1/memory/runtime", handlers.MemoryRuntime)
	...
	mux.HandleFunc("/api/v1/controls", handlers.ControlsCatalog)
	mux.HandleFunc("/api/v1/controls/status", handlers.StatusSummary)
	mux.HandleFunc("/api/v1/controls/auth", handlers.AuthSummary)
	mux.HandleFunc("/api/v1/controls/plugins", handlers.PluginsSummary)
	mux.HandleFunc("/api/v1/doctor", handlers.Doctor)
	mux.HandleFunc("/api/v1/controls/models", handlers.ModelsSummary)
	mux.HandleFunc("/api/v1/controls/channels", handlers.ChannelsSummary)
	mux.HandleFunc("/api/v1/controls/channels/action", handlers.ChannelsAction)
	mux.HandleFunc("/api/v1/controls/tools", handlers.ToolsSummary)
	mux.HandleFunc("/api/v1/controls/approvals", handlers.ApprovalsSummary)
	mux.HandleFunc("/api/v1/controls/logs", handlers.LogsSummary)
	mux.HandleFunc("/api/v1/controls/browser", handlers.BrowserSummary)
	mux.HandleFunc("/api/v1/controls/sessions", handlers.SessionsSummary)
	mux.HandleFunc("/api/v1/controls/cron", handlers.CronSummary)
	mux.HandleFunc("/api/v1/controls/sessions/action", handlers.SessionsAction)
	mux.HandleFunc("/api/v1/controls/cron/action", handlers.CronAction)
	mux.HandleFunc("/api/v1/controls/models/action", handlers.ModelAction)
	mux.HandleFunc("/api/v1/controls/mcp/action", handlers.MCPServerAction)
	mux.HandleFunc("/api/v1/controls/browser/action", handlers.BrowserAction)
	mux.HandleFunc("/api/v1/controls/approvals/action", handlers.ApprovalsAction)
	mux.HandleFunc("/api/v1/controls/tools/exec-profile", handlers.ExecProfileAction)
	...
	mux.HandleFunc("/api/v1/beliefs", handlers.Beliefs)
	mux.HandleFunc("/api/v1/chat", handlers.Chat)
	mux.HandleFunc("/api/v1/delegate", handlers.Delegate)
	...
	// begin: Codex-CLI-compatible flow — starts local server on :1455, returns auth URL
	mux.HandleFunc("/api/v1/auth/openai/begin", handlers.OpenAIBeginAuthFlow)
	// File serving: /api/v1/files/<relative-path> → ~/.openclio/output/<relative-path>
	mux.HandleFunc("/api/v1/files/upload", handlers.UploadFile)
	mux.HandleFunc("/api/v1/files/", handlers.ServeOutputFile)
	...
	mux.HandleFunc("/api/v1/setup/disconnect", handlers.DisconnectProvider)
...
// AttachMemoryProvider wires an optional tier-3 memory provider used for context assembly.
func (s *Server) AttachMemoryProvider(provider agentctx.MemoryProvider) {
	if s == nil || s.handlers == nil {
		return
	}
	s.handlers.AttachMemoryProvider(provider)
}
...
func (s *Server) AttachBeliefStore(store eam.BeliefStore) {
	if s == nil || s.handlers == nil {
		return
	}
	s.handlers.AttachBeliefStore(store)
}
