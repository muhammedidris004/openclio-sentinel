...
type Agent struct {
	...
	hookMu             sync.RWMutex
	beforeAssembleHook func(context.Context, string) error
	afterResponseHook  func(ctx context.Context, sessionID, userMsg, assistantMsg string)
	eventEmitter       func(AgentEvent)
	emitterMu          sync.Mutex
}

func normalizeMaxIterations(iters int) int {
	if iters < 0 {
		return 0
	}
	return iters
...
func (a *Agent) hasIterationLimit() bool {
	return a.maxIterations > 0
}
...
// SetAgentName sets the display name used in the system prompt.
// Defaults to "openclio" if never called.
func (a *Agent) SetAgentName(name string) {
	if n := strings.TrimSpace(name); n != "" {
		a.agentName = n
	}
}

func (a *Agent) resolvedAgentName() string {
	if a.agentName != "" {
		return a.agentName
	}
	return "openclio"
...
// reasoningFields returns thinking-related fields for a ChatRequest.
// When reasoning is enabled:
//   - ThinkingBudget is set (min 1024; defaults to 8000)
//   - MaxTokens is bumped to budget + 8192 so there is room for the actual reply
//     on top of the thinking tokens (Anthropic requires max_tokens > budget_tokens)
func (a *Agent) reasoningFields() (thinking bool, budget, maxTokens int) {
	if !a.cfg.ReasoningEnabled {
		return false, 0, 4096
	}
	budget = a.cfg.ReasoningBudget
	if budget < 1024 {
		budget = 8000
	}
	// max_tokens must exceed budget to leave room for the actual response
...
// SetBeforeAssembleHook sets an optional hook run before context assembly.
func (a *Agent) SetBeforeAssembleHook(hook func(context.Context, string) error) {
	a.hookMu.Lock()
	defer a.hookMu.Unlock()
	a.beforeAssembleHook = hook
}

func (a *Agent) runBeforeAssembleHook(ctx context.Context, sessionID string) error {
	a.hookMu.RLock()
	hook := a.beforeAssembleHook
	a.hookMu.RUnlock()
	if hook == nil {
		return nil
	}
...
func (a *Agent) warn(ctx context.Context, msg string, args ...any) {
	logger.Global.WarnContext(ctx, msg, args...)
}

// SetAfterResponseHook registers a callback invoked asynchronously after every
// complete agent response. It is safe to call from multiple goroutines.
func (a *Agent) SetAfterResponseHook(hook func(ctx context.Context, sessionID, userMsg, assistantMsg string)) {
	a.hookMu.Lock()
	defer a.hookMu.Unlock()
	a.afterResponseHook = hook
}

func (a *Agent) runAfterResponseHook(ctx context.Context, sessionID, userMsg, assistantMsg string) {
	a.hookMu.RLock()
	hook := a.afterResponseHook
	a.hookMu.RUnlock()
	if hook == nil {
		return
	}
	// Always async — never blocks the user waiting for a response.
	go hook(context.Background(), sessionID, userMsg, assistantMsg)
}

func (a *Agent) resolveMemoryProvider(memProvider agentctx.MemoryProvider) agentctx.MemoryProvider {
	if memProvider != nil {
		return memProvider
	}
	if a.workspace == nil {
	...
	return memoryserving.NewStaticProvider(a.workspace.Memory)
}
...
func NewAgent(provider Provider, contextEngine *agentctx.Engine, toolExecutor ToolExecutor, cfg config.AgentConfig, model string) *Agent {
	return &Agent{
		...
		model:         model,
		maxIterations: normalizeMaxIterations(cfg.MaxToolIterations),
	}
}
...
// MemoryWriter is an optional extension for tier-3 memory providers.
...
func (a *Agent) Run(ctx context.Context, sessionID, userMessage string,
	...
	systemPrompt := BuildSystemPrompt(a.resolvedAgentName(), identity, userCtx, a.gitContext, toolNames)
	...
	// Assemble context
	if err := a.runBeforeAssembleHook(ctx, sessionID); err != nil {
		a.warn(ctx, "pre-assemble hook failed", "session_id", sessionID, "error", err)
	}
	memProvider = a.resolveMemoryProvider(memProvider)
	...
	for iteration := 0; !a.hasIterationLimit() || iteration < a.maxIterations; iteration++ {
		...
		thinking, thinkingBudget, maxTokens := a.reasoningFields()
		chatReq := ChatRequest{
		...
		if len(chatResp.ToolCalls) == 0 {
			...
			response.Duration = time.Since(start)
			a.runAfterResponseHook(ctx, sessionID, userMessage, response.Text)
			return response, nil
		}
...
// emit fires an AgentEvent to the registered eventEmitter, if any.
...
// onEvent receives real-time delegation events when the delegate tool is used;
...
func (a *Agent) maybeTriggerCompaction(
	...
	go func() {
		...
		if err := a.runCompaction(ctx, sessionID, msgProvider, memProvider); err != nil {
			a.warn(ctx, "proactive compaction failed", "session_id", sessionID, "error", err)
		}
	}()
}
