...
// Routes Codex models to the Responses API; all others use Chat Completions.
func (p *OpenAIProvider) Stream(ctx context.Context, req ChatRequest) (<-chan StreamChunk, error) {
	...
	if isCodexResponsesModel(model) {
		return p.streamViaResponsesAPI(ctx, req)
	}

	msgs, tools, _, maxTokens := p.buildRequest(req)
