...
type ContextStats struct {
	...
	SemanticMemoryTokens   int `json:"semantic_memory_tokens"` // Tier 3 only
	KnowledgeGraphTokens   int `json:"knowledge_graph_tokens"` // Tier 4 only
	KnowledgeNodesCount    int `json:"knowledge_nodes_count"`  // Tier 4 nodes included
...
// QueryMemoryProvider is an optional extension for semantic memory sources that
// can filter memories using the current user message.
type QueryMemoryProvider interface {
	GetMemoriesForQuery(query string, limit int) ([]string, error)
}

// PromptMemoryProvider optionally augments the system prompt with
// token-budgeted memory blocks (for example beliefs and knowledge gaps).
type PromptMemoryProvider interface {
	BuildPromptAddendum(sessionID, userMessage string, maxTokens int) (string, error)
}
...
func (e *Engine) Assemble(
	...
	remainingSemantic := alloc.SemanticMemory

	if memProvider != nil {
		if promptProvider, ok := memProvider.(PromptMemoryProvider); ok && remainingSemantic > 0 {
			addendum, err := promptProvider.BuildPromptAddendum(sessionID, userMessage, remainingSemantic)
			if err != nil {
				fmt.Printf("warning: building prompt memory addendum failed: %v\n", err)
			} else {
				addendum = strings.TrimSpace(addendum)
				if addendum != "" {
					addendumTokens := EstimateTokens(addendum)
					if addendumTokens <= remainingSemantic {
						systemPrompt = strings.TrimSpace(systemPrompt + "\n\n" + addendum)
						stats.SemanticMemoryTokens += addendumTokens
						remainingSemantic -= addendumTokens
					} else {
						fmt.Printf(
							"warning: prompt memory addendum exceeds semantic budget (%d > %d), skipping\n",
							addendumTokens,
							remainingSemantic,
						)
					}
	...
	if memProvider != nil {
		...
		if queryAware, ok := memProvider.(QueryMemoryProvider); ok {
			memories, err = queryAware.GetMemoriesForQuery(userMessage, 20)
		} else {
		...
		if err != nil {
			...
			for _, mem := range memories {
				...
				if memoryTokensUsed+memTokens > remainingSemantic {
					break
				}
				memoryContent += mem + "\n"
			...
			if memoryContent != "" {
				...
				remainingSemantic -= memoryTokensUsed
			}
		}
	}
	...
	if kp, ok := msgProvider.(KnowledgeProvider); ok {
		...
		collectKnowledge:
			...
			if len(lines) > 0 {
				...
				stats.KnowledgeGraphTokens = knowledgeTokensUsed
				stats.KnowledgeNodesCount = len(lines)
			}
		}
	}
	...
	stats.TotalTokens = stats.SystemPromptTokens + stats.UserMessageTokens +
		...
		stats.SemanticMemoryTokens + stats.KnowledgeGraphTokens + stats.ToolDefTokens
