...
func NewExecTool(cfg config.ExecToolConfig, workspaceDir string, maxOutputSize int, scrubOutput bool) *ExecTool {
	...
	return &ExecTool{
		...
		allowedCommands:     buildAllowedCommandsSet(cfg.AllowedCommands),
		approvalOnBlock:     cfg.ApprovalOnBlock,
	}
}
...
func (t *ExecTool) Execute(ctx context.Context, params json.RawMessage) (string, error) {
	...
	if ok, reason := t.isCommandAllowed(p.Command); !ok {
		if t.approvalOnBlock {
			approved, err := t.confirmBlockedCommand(p.Command, reason)
			if err != nil {
				t.recordAction(p.Command, p.WorkDir, "", err)
				return "", err
			}
			if !approved {
				err := fmt.Errorf("command blocked by exec profile: %s", reason)
				t.recordAction(p.Command, p.WorkDir, "", err)
				return "", err
			}
		} else {
...
func (t *ExecTool) confirmBlockedCommand(command, reason string) (bool, error) {
	if stat, err := os.Stdin.Stat(); err != nil || (stat.Mode()&os.ModeCharDevice == 0) {
		return false, fmt.Errorf("command approval requires an interactive terminal")
	}
	fmt.Printf("⚠️ Command not in exec profile (%s).\n  Reason: %s\n  $ %s\n\nReply 'yes' to allow once, 'no' to deny: ", t.profile, reason, command)
	reader := bufio.NewReader(os.Stdin)
	line, err := reader.ReadString('\n')
	if err != nil {
		return false, fmt.Errorf("command approval unavailable: %w", err)
	}
	switch strings.ToLower(strings.TrimSpace(line)) {
	case "yes", "y":
		return true, nil
...
func (t *ExecTool) isCommandAllowed(command string) (bool, string) {
	if len(t.allowedCommands) == 0 {
		return true, ""
	}
	name := firstCommandName(command)
	if name == "" {
		return false, "unable to determine command name"
	}
	if _, ok := t.allowedCommands[name]; ok {
		return true, ""
	}
	return false, fmt.Sprintf("%q is not in the %s exec profile allowlist", name, t.profile)
}

func buildAllowedCommandsSet(commands []string) map[string]struct{} {
	if len(commands) == 0 {
		return nil
	}
...
func firstCommandName(command string) string {
	...
	for _, field := range fields {
		if strings.Contains(field, "=") && !strings.HasPrefix(field, "/") && !strings.HasPrefix(field, ".") {
			if parts := strings.SplitN(field, "=", 2); len(parts) == 2 && parts[0] != "" {
				continue
			}
		}
