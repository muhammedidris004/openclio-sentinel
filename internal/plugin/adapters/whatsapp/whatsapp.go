...
// HasStoredSession reports whether a prior linked WhatsApp session exists.
func (a *Adapter) HasStoredSession() bool {
	return a != nil && a.client != nil && a.client.Store != nil && a.client.Store.ID != nil
}
...
func (a *Adapter) Health() error {
	...
	case "waiting_for_qr", "code", "success":
		return nil
	}
...
func (a *Adapter) Start(ctx context.Context, inbound chan<- plugin.InboundMessage, outbound <-chan plugin.OutboundMessage) error {
	...
	if a.client.Store.ID == nil {
		...
		for evt := range qrChan {
			if evt.Event == "code" {
				...
				// Mark QR success immediately; we still wait for an actual client
				// connection before advertising the channel as connected.
				pairingSucceeded = true
				a.setQRState("success", "")
		...
		if !pairingSucceeded {
			a.setQRState("pairing_incomplete", "")
			return fmt.Errorf("whatsapp: QR pairing did not complete")
		}
	} else {
	...
	if !a.waitForConnection(ctx, 10*time.Second) {
		a.setQRState("connect_timeout", "")
		return fmt.Errorf("whatsapp: connect did not reach healthy state")
	}
...
func (a *Adapter) waitForConnection(ctx context.Context, timeout time.Duration) bool {
	if a.client == nil {
		return false
	}
