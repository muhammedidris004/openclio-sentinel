...
Every LLM call has a token budget. The 4-tier context engine (working memory → episodic retrieval → semantic facts → knowledge graph) sends only what is relevant, not everything. Tool results are auto-summarized. System prompts are compressed and cached. At 50 conversation turns, openclio uses ~87% fewer input tokens than a naive implementation.
...
- 4-tier context engine with SQLite vector search and knowledge graph context
- CLI chat interface
- YAML config system
