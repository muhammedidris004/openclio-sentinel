...
// WithMemoryProvider attaches an optional tier-3 memory provider for routed runs.
func (r *Router) WithMemoryProvider(provider agentctx.MemoryProvider) *Router {
	r.memory = provider
	return r
}
...
func (r *Router) handleMessage(ctx context.Context, msg InboundMessage) {
	...
	resp, err := r.agentInstance.Run(ctx, sessionID, msg.Text, msgProvider, r.memory)
	if err != nil {
