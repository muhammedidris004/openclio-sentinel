...
// SetSwitchModelPersister wires a ConfigModelPersister to the switch_model tool
// so model changes are saved to config.yaml and survive a restart.
func (r *Registry) SetSwitchModelPersister(p ConfigModelPersister) {
	r.mu.RLock()
	t, ok := r.tools["switch_model"]
	r.mu.RUnlock()
	if !ok {
		return
	}
	if smt, ok := t.(*SwitchModelTool); ok {
		smt.SetConfigPersister(p)
	}
}
