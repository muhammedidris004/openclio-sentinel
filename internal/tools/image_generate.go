...
func (t *ImageGenerateTool) Execute(ctx context.Context, params json.RawMessage) (string, error) {
	...
	for i, d := range apiResp.Data {
		...
		// Relative web URL served by the gateway's /api/v1/files/ endpoint
		webURLs = append(webURLs, "/api/v1/files/imagegen/"+filename)
	}

	// Build markdown snippets so the agent can embed images directly in responses
	var mdSnippets []string
	for i, u := range webURLs {
		mdSnippets = append(mdSnippets, fmt.Sprintf("![generated image %d](%s)", i+1, u))
	}

	out, _ := json.Marshal(map[string]any{
		"files":    written,
		"urls":     webURLs,
		"markdown": strings.Join(mdSnippets, "\n"),
		"count":    len(written),
		"dir":      outDir,
	})
