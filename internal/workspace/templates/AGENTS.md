...
3. Manages a 4-tier context engine for token-efficient memory
...
├── context/          4-tier memory engine
│   ├── context.go    Main context assembler (tiers 1-4)
│   ├── budget.go     Token budget allocator
│   ├── search.go     Tier 2 similarity search helpers
│   ├── embeddings.go Embedding providers
│   └── tokens.go     Token estimation utilities
├── memory/           Memory services and providers
│   ├── store.go      Generic memory storage module
│   └── serving/      Runtime memory providers (workspace/static)
...
User sends message
  ...
  Agent Loop (internal/agent/loop.go)
        ...
        │         └── Tier 4: relevant entities (knowledge graph)
        │
